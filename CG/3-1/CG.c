...
void LoadObj(const char* filepath) {
    ...
    while (fgets(line, sizeof(line), fp)) {
        if (line[0] == 'v' && line[1] == ' ') {
            ...
            int v[4], vn;
            int matched = sscanf(line, "f %d//%d %d//%d %d//%d %d//%d", &v[0], &vn, &v[1], &vn, &v[2], &vn, &v[3], &vn);
            if (matched == 6) { // 3頂点の面
                ...
                f_count++;
            } else if (matched == 8) { // 4頂点の面
                faces[f_count][0] = v[0] - 1;
                faces[f_count][1] = v[1] - 1;
                faces[f_count][2] = v[2] - 1;
                faces[f_count][3] = v[3] - 1;
                face_normals[f_count] = vn - 1;  // 法線インデックス
                f_count++;
            }
        }
...
void PrintObjData() {
    ...
    for (int i = 0; i < f_count; i++) {
        ...
        for (int j = 0; j < 3; j++) {  // 各面は3頂点を持つ
            printf("%d//%d ", faces[i][j] + 1, face_normals[i] + 1);
        }
        printf("\n");
    }
}
...
int main(int argc, char** argv) {
    ...
    glutCreateWindow("Pyramid with Different Colors");
